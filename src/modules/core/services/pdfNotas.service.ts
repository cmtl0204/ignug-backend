...
export class PDFNotas {
  ...
  async generarPDF():Promise<Buffer> {
    ...
    console.log(data.enrollment.student.user) 
    const pdfBuffer: Buffer = await new Promise(resolve => {
      ...
      doc.font('Times-Roman', 13).text(`El estudiante ${data.enrollment.student.user.name} con la cédula ${data.enrollment.student.user.identification} en ${data.enrollment.academicPeriod.name} periódo académico a obtenido las siguientes calificaciones`, 80, 280, { align: 'justify' })

      const rows_grades = [];
        const list = [data.subject.name,data.finalGrade,data.finalGrade,data.finalAttendance,data.finalAttendance,data.finalGrade,data.finalAttendance,data.academicState.name];
        rows_grades.push(list);
     
      const table = {
        headers: ['Asignaturas', 'Nota Parcial 1', 'Nota Parcial 2', 'Asistencia Parcial 1', 'Asistencia Parcial 2', 'Nota Final', 'Asistencia Final', 'Estado Académico'],
        rows: rows_grades,
      };
