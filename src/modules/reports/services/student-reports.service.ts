...
export class StudentReportsService {
  ...
  async generateStudentCard(@Res() res: Response, id: string, careerId: string, schoolPeriodId: string) {
    ...
    try {
      ...
      doc.pipe(res);
      console.log(doc.page.width - 300);
      console.log(doc.page.height);

      doc.image(avatarPath, 180, 130, {
        width: 260,
        height: 280,
      });

      doc.image('./resources/images/reports/student-card.png', 50, 0, {
      ...
      doc.font('Helvetica').fontSize('28').fillColor('#0082CB');
      // .fillColor('white');
