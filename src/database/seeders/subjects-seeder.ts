import {Injectable} from '@nestjs/common';
import * as XLSX from 'xlsx';
import {faker} from '@faker-js/faker';
import {join} from 'path';
import {SeedSubjectDto} from '@core/dto';
import {CatalogueEntity, CurriculumEntity} from '@core/entities';
import {
    CataloguesService,
    CurriculumsService,
    SubjectCorequisitesService,
    SubjectPrerequisitesService,
    SubjectsService
} from '@core/services';
import {CatalogueTypeEnum} from '@shared/enums';

@Injectable()
export class SubjectsSeeder {
    private states: CatalogueEntity[] = [];
    private periods: CatalogueEntity[] = [];
    private types: CatalogueEntity[] = [];
    private curriculums: CurriculumEntity[] = [];

    constructor(
        private subjectService: SubjectsService,
        private subjectCorequisitesService: SubjectCorequisitesService,
        private subjectRequirementsService: SubjectPrerequisitesService,
        private catalogueService: CataloguesService,
        private curriculumsService: CurriculumsService,
    ) {
    }

    async run() {
        await this.loadCatalogues();
        await this.loadCurriculums();
        await this.createSubjects();
        await this.createCorequisites();
        await this.createPrerequisites();
    }

    private async loadCatalogues() {
        const catalogues = (await this.catalogueService.findAll()).data as CatalogueEntity[];

        this.states = catalogues.filter(catalogue => catalogue.type === CatalogueTypeEnum.SUBJECTS_STATE);

        this.periods = catalogues.filter(catalogue => catalogue.type === CatalogueTypeEnum.ACADEMIC_PERIOD);

        this.types = catalogues.filter(catalogue => catalogue.type === CatalogueTypeEnum.SUBJECTS_TYPE);
    }

    private async loadCurriculums() {
        this.curriculums = (await this.curriculumsService.findAll()).data as CurriculumEntity[];
    }

    private async createSubjects() {
        const subjects: SeedSubjectDto[] = [];

        const stateEnabled = this.states.find((state: CatalogueEntity) => {
            return state.code === 'enabled' && state.type === CatalogueTypeEnum.SUBJECTS_STATE;
        });

        //Periodo academico
        const leveling = this.periods.find((period: CatalogueEntity) => {
            return period.code === 'leveling' && period.type === CatalogueTypeEnum.ACADEMIC_PERIOD;
        });
        const first = this.periods.find((period: CatalogueEntity) => {
            return period.code === '1' && period.type === CatalogueTypeEnum.ACADEMIC_PERIOD;
        });
        const second = this.periods.find((period: CatalogueEntity) => {
            return period.code === '2' && period.type === CatalogueTypeEnum.ACADEMIC_PERIOD;
        });
        const third = this.periods.find((period: CatalogueEntity) => {
            return period.code === '3' && period.type === CatalogueTypeEnum.ACADEMIC_PERIOD;
        });
        const fourth = this.periods.find((period: CatalogueEntity) => {
            return period.code === '4' && period.type === CatalogueTypeEnum.ACADEMIC_PERIOD;
        });
        const fifth = this.periods.find((period: CatalogueEntity) => {
            return period.code === '5' && period.type === CatalogueTypeEnum.ACADEMIC_PERIOD;
        });

        //curriculum
        const curriculum1 = this.curriculums.find((curriculum: CurriculumEntity) => curriculum.code === 'cod1');
        const curriculum2 = this.curriculums.find((curriculum: CurriculumEntity) => curriculum.code === 'cod2');

        //tipo asignatura
        const type = this.types.find(type => {
            return type.code === 'subject' && type.type === CatalogueTypeEnum.SUBJECTS_TYPE;
        });

        const levelingType = this.types.find(type => {
            return type.code === 'leveling' && type.type === CatalogueTypeEnum.SUBJECTS_TYPE;
        });

        subjects.push(
            {
                academicPeriod: leveling,
                curriculum: curriculum1,
                type: levelingType,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'leveling1',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura Nivelación 1',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: leveling,
                curriculum: curriculum1,
                type: levelingType,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'leveling2',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura Nivelación 2',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: leveling,
                curriculum: curriculum1,
                type: levelingType,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'leveling3',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura Nivelación 3',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: leveling,
                curriculum: curriculum1,
                type: levelingType,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'leveling4',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura Nivelación 4',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: leveling,
                curriculum: curriculum1,
                type: levelingType,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'leveling5',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura Nivelación 5',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: first,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod1',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 1',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: first,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod2',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 2',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: first,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod3',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 3',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: first,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod4',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 4',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: second,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod5',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 5',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: second,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod6',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 6',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: second,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod7',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 7',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: second,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod8',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 8',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: third,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod9',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 9',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: third,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod10',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 10',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: third,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod11',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 11',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: third,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod12',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 12',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fourth,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod13',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 13',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fourth,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod14',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 14',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fourth,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod15',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 15',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fourth,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod16',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 16',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fifth,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod17',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 17',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fifth,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod18',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 18',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fifth,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod19',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 19',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fifth,
                curriculum: curriculum1,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod20',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 20',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: first,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod21',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 21',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: first,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod22',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 22',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: first,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod23',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 23',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: first,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod24',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 24',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: second,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod25',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 25',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: second,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod26',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 26',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: second,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod27',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 27',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: second,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod28',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 28',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: third,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod29',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 29',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: third,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod30',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 30',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: third,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod31',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 31',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: third,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod32',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 32',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fourth,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod33',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 33',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fourth,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod34',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 34',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fourth,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod35',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 35',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fourth,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod36',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 36',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fifth,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod37',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 37',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fifth,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod38',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 38',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fifth,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod39',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 39',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
            {
                academicPeriod: fifth,
                curriculum: curriculum2,
                type: type,
                state: stateEnabled,
                autonomousHour: faker.number.int({min: 10, max: 50}),
                code: 'cod40',
                credits: faker.number.int({min: 50, max: 160}),
                isVisible: true,
                isEnabled:true,
                name: 'Asignatura 40',
                practicalHour: faker.number.int({min: 20, max: 80}),
                scale: 1,
                teacherHour: faker.number.int({min: 20, max: 80}),
                subjectPrerequisites: [],
                subjectCorequisites: [],
            },
        );

        for (const subject of subjects) {
            await this.subjectService.create(subject);
        }
    }

    private async createCorequisites() {
        const workbook = XLSX.readFile(join(process.cwd(), 'src/database/seeders/files/subject_corequisites.xlsx'));

        const workbookSheets = workbook.SheetNames;
        const sheet = workbookSheets[0];
        const dataExcel = XLSX.utils.sheet_to_json(workbook.Sheets[sheet]);

        for (const item of dataExcel) {
            const subject = await this.subjectService.findByCode(item['subject_code']);
            const requirement = await this.subjectService.findByCode(item['requirement_code']);

            await this.subjectCorequisitesService.createCorequisite({
                subject,
                requirement,
                isEnabled: true,
            });
        }

        return true;
    }

    private async createPrerequisites() {
        const workbook = XLSX.readFile(join(process.cwd(), 'src/database/seeders/files/subject_prerequisites.xlsx'));

        const workbookSheets = workbook.SheetNames;
        const sheet = workbookSheets[0];
        const dataExcel = XLSX.utils.sheet_to_json(workbook.Sheets[sheet]);

        for (const item of dataExcel) {
            const subject = await this.subjectService.findByCode(item['subject_code']);
            const requirement = await this.subjectService.findByCode(item['requirement_code']);

            await this.subjectRequirementsService.createPrerequisite({
                subject,
                requirement,
                isEnabled: true,
            });
        }

        return true;
    }
}
